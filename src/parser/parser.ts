...
function getReferences(input, regex) {
  ...
  // console.log('getReferences', input, regex, reApplicable)
...
function parser(raw, options, regex) {
  ...
  let isBody = true
  const headerCorrespondence = map(options.headerCorrespondence, function (part) {
    return part.trim()
  })
  const revertCorrespondence = map(options.revertCorrespondence, function (field) {
    return field.trim()
  })
  const mergeCorrespondence = map(options.mergeCorrespondence, function (field) {
    return field.trim()
  })
  ...
  if (mergeMatch && options.mergePattern) {
    ...
    forEach(mergeCorrespondence, function (partName, index) {
    ...
    forEach(mergeCorrespondence, function (partName) {
  ...
  if (headerMatch) {
    forEach(headerCorrespondence, function (partName, index) {
      const partValue = headerMatch[index + 1] || null
      headerParts[partName] = partValue
    })
  } else {
    forEach(headerCorrespondence, function (partName) {
  ...
  if (revertMatch) {
    revert = {}
    forEach(revertCorrespondence, function (partName, index) {
